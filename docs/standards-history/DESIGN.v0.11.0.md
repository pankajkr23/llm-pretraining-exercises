> **FROZEN COPY — NOT IN FORCE.** This is `docs/DESIGN.md` exactly as it shipped in **v0.11.0**, kept
> so a rewrite can be diffed against what it replaced. It is history, not policy: the live
> file at `docs/DESIGN.md` is the one in force, and this copy is never edited. Snapshots are
> written by `tools/snapshot_standards.py` and checked by `tests/test_standards_history.py`.
