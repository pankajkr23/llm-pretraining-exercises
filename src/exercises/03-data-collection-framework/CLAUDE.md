...
- **The pages carry interactive explainers, not widgets.** An explainer makes one claim, and the
  ...
  holds twelve chapters, one per reader question, each in three layers — headline, interaction, and a
  closed "The arithmetic". `web/report/` and `web/reasoning/` are redirect stubs from the two-page
  ...
  `docs/DESIGN_CRITIQUE.md`; the rebuild order is `docs/NEW_TODO.md`.
