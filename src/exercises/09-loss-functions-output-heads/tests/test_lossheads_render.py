...
# `test_no_prose_table_hides_a_cell_behind_its_own_scrollbar` lived here and is now
# `tests/test_prose_tables_wrap.py`, which sweeps every deployed page at four widths rather than
# this one at one. Promoting it was not housekeeping: exercise 10 copied this page's `grid prose`
# without the stylesheet rule that makes it mean anything, and a guard that only ever looked here
# could not see a 3,635px table in a 1,156px wrapper one directory over.
