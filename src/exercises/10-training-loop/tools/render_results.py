...
def main() -> int:
    ...
    if page_data.parent.is_dir():
        page_data.write_text(render_page_data(run))
        print(f"wrote web/{page_data.name}")
    return 0


def render_page_data(run: dict) -> str:
    """Generate `web/data.js` — every figure the page draws, read from the same run.

    **The page must not hold a number of its own.** Exercise 09 shipped a document claiming every
    figure was generated while fifteen were typed inside its renderer, and a page is the version of
    that failure the most people see. So `chapters.js` reads `M.*` and writes nothing.
    """
    return (
        "/* GENERATED by tools/render_results.py. Do not edit.\n"
        " *\n"
        " * Every number the page draws is in here, read from results/run.json. `chapters.js`\n"
        " * holds none of its own.\n"
        " */\n"
        "export const M = " + json.dumps(run, indent=2, sort_keys=True) + ";\n"
    )
