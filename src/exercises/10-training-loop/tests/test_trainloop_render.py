"""Exercise 10's page, tested in a browser, because `node --check` proves almost nothing about it.
...
def test_no_token_label_overflows_the_box_it_sits_in(page):
    ...
    Exercise 09's Figure 1 shipped with a guessed cell width and five of seven token strings
    rendered past the edge of their own rounded box. Nothing caught it:
    ...
    overflows = page.evaluate(
        """() => {
             const out = [];
             for (const svg of document.querySelectorAll('main svg.accfig')) {
