...
def test_when_you_would_pick_it_reaches_the_reader_for_every_mechanism(page) -> None:
    ...
    `whenToChoose` is on all thirty catalogue entries and the page rendered it exactly once — inside
    the reading spread, which shows one mechanism at a time and only after a click. So the most
    decision-relevant thing in the catalogue was the least reachable thing on the page, and a reader
    could not compare any two of them.

    It briefly got a table of its own near the top of the page. That was the wrong fix and a reader
    caught it: a second thirty-row table of the same catalogue is 978 words of duplication, and the
    field simply belonged in the table that already existed. The guard is written against the
    *property* — the text is in the document, no click required — so it survived the table being
    deleted with only its selector changed.
    """
    bundle = _bundle()
    picks = page.eval_on_selector_all("#reproduce .ix-pick", "els => els.map(e => e.textContent)")
    assert picks, "no when-you-would-pick-it cells rendered at all"
    expected = [m["whenToChoose"] for m in bundle["mechanisms"]]
    missing = [e for e in expected if not any(e in p for p in picks)]
    assert not missing, f"whenToChoose never reaches the reader for: {missing[:2]}"


def test_the_catalogue_is_tabulated_exactly_once(page) -> None:
    """One table of the thirty, not two. A reader found the second one and called it duplication.

    The index at the back carries every mechanism with every field. A second thirty-row table was
    added near the top to surface `whenToChoose`, and it restated the date, the name, the bill and
    the shipped-in column that the index already had — 43% of the page became two tables of the same
    catalogue. The field belonged in the existing table.

    This asserts the property rather than naming the deleted section: no two containers on the page
    may each hold a row per mechanism. A future table of all thirty is a decision someone has to
    make deliberately, not one that can be added on top of the index without noticing.
    """
    counts = page.evaluate(
        r"""(total) => {
          const out = [];
          const sel = '#main div, #main section, #main table, #main ul';
          for (const e of document.querySelectorAll(sel)) {
            // Direct children only: a wrapper around the index must not count as a second table.
            const rows = [...e.children].filter(c => c.id && /^m-/.test(c.id)).length;
            if (rows >= total) out.push(e.className || e.tagName);
          }
          return out;
        }""",
        len(_bundle()["mechanisms"]),
    )
    assert len(counts) == 1, (
        f"the catalogue is tabulated {len(counts)} times on one page: {counts}. "
        "One table with every field beats two tables that restate each other."
    )
...
@pytest.mark.parametrize("width", [1440, 1180, 900, 390])
def test_no_sentence_on_the_page_is_silently_cut_off(page, width: int) -> None:
    """`overflow: hidden` plus `white-space: nowrap` truncates with no ellipsis and no warning.

    The invoice's cut line — the sentence the whole figure exists to deliver — read "…the cache
    alone needs a second ma" at every width narrower than the sentence, for as long as the figure
    has existed. `test_the_invoice_cut_line_is_visible` passed the entire time, because *visible*
    and *legible* are different assertions and only one of them was being made.

    This asserts the general property: no element whose text is clipped by its own box. A 1px
    tolerance absorbs sub-pixel rounding, which is real and would otherwise make this flap.
    """
        ...
        clipped = page.evaluate(
            """() => {
              ...
              for (const e of document.querySelectorAll('#main *')) {
                ...
                if (cs.overflow === 'auto' || cs.overflowX === 'auto') continue;
                if (e.scrollWidth > e.clientWidth + 1) {
                  bad.push(e.className + ': ' + e.textContent.trim().slice(0, 60));
                }
              }
              return bad;
            }"""
        )
        assert not clipped, f"text cut off by its own box at {width}px: {clipped[:3]}"
    finally:
...
def test_all_four_families_are_explained_without_touching_the_control(page) -> None:
    """A reader: the four families were "a bit challenging… rather than just prose".

    The fix is an interactive figure — one score grid drawn four ways — and the risk an interactive
    figure always carries is that it becomes the ONLY route to the lesson. `AGENTS.md` forbids that
    outright: a print reader, a reduced-motion reader and a reader who never touches the control
    must still get the whole taxonomy.

    So the tabs change the drawing and the four plain-language rows are always in the document. This
    asserts every family's analogy is present and visible on load, before any click.
    """
    rows = page.eval_on_selector_all(
        ".fam-rows .fam-row",
        """els => els.map(e => ({
             label: e.querySelector('.fam-lab').textContent,
             words: (e.querySelector('p').textContent.match(/\\S+/g) || []).length,
             visible: e.getBoundingClientRect().height > 0,
           }))""",
    )
    kinds = sorted(_bundle()["counts"]["glyphKinds"])
    assert sorted(r["label"].split(" ")[0].lower() for r in rows) == kinds
    for r in rows:
        assert r["visible"], f"{r['label']} is not rendered"
        assert r["words"] >= 20, f"{r['label']} has no real explanation: {r['words']} words"


def test_switching_family_changes_the_drawing(page) -> None:
    """A figure whose tabs all draw the same thing is a control that teaches nothing.

    This exercise has shipped that exact defect before; the centrefold carries its own version of
    this guard for the same reason. Four families, four different pictures.
    """
    shapes = []
    for i in range(4):
        page.eval_on_selector_all(".fam .tabs button", f"els => els[{i}].click()")
        page.wait_for_timeout(120)
        shapes.append(
            page.eval_on_selector(
                ".fam .fam-svg",
                """e => [...e.querySelectorAll('rect, path')]
                       .filter(n => n.getBoundingClientRect().height > 0)
                       .map(n => n.getAttribute('class') + ':' + n.getAttribute('opacity'))
                       .join('|')""",
            )
        )
    assert len(set(shapes)) == 4, "two of the four families draw the same picture"
...
def test_the_key_draws_one_exemplar_per_glyph_family_that_the_plate_uses(page) -> None:
    ...
    plate does not draw."""
    bundle = _bundle()
    # The selector has moved twice now — out of `#glossary`, then out of `.key-alpha` when the
    # alphabet merged into the four-families figure so each mark sits beside the analogy that
    # explains it. The PROPERTY is what is being tested and it has not changed: one exemplar per
    # family, drawn by the same generator the chronology uses. The emptiness assertion is what stops
    # a stale selector turning this into a guard that passes on an empty page — it has earned its
    # keep both times.
    shown = page.eval_on_selector_all(
        ".fam-head .fam-lab", "els => els.map(e => e.textContent.split(' ')[0])"
    )
    assert shown, "no family exemplars rendered at all"
