...
def test_the_key_draws_one_exemplar_per_glyph_family_that_the_plate_uses(page) -> None:
    ...
    plate does not draw."""
    bundle = _bundle()
    # NOT scoped to `#glossary`. The key now sits above the chronology, where the glyphs are first
    # used at size, and a selector naming the old section matched nothing — one assertion away from
    # a guard that passes on an empty page. The emptiness check is the difference, and it holds
    # whatever section the key ends up in.
    shown = page.eval_on_selector_all(
        ".key-alpha .it .lab", "els => els.map(e => e.textContent.split(' ')[0])"
    )
    assert shown, "the alphabet rendered no entries at all"
