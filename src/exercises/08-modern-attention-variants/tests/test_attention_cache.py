...
def test_the_yardstick_is_the_one_the_notes_used() -> None:
    ...
    assert YARDSTICK.bytes_per_number == 2
...
def test_eight_users_at_32k_costs_what_the_notes_say() -> None:
    """And eight of them is 51.54 GB — the figure that makes the point about serving."""
    got = kv_cache_bytes(YARDSTICK, context=32_768, batch=8)
    assert round(got / GB, 2) == 51.54
    assert got == 8 * kv_cache_bytes(YARDSTICK, context=32_768, batch=1)
...
def test_the_sharing_ladder_only_moves_kv_heads() -> None:
    ...
    ladder = {s.name: s for s in sharing_ladder(YARDSTICK)}
    assert ladder["MQA"].kv_heads == 1
    assert ladder["MHA"].kv_heads == YARDSTICK.query_heads
...
def test_sharing_and_compression_multiply_rather_than_compete() -> None:
    ...
    """
    full = kv_cache_bytes(YARDSTICK, context=32_768, kv_heads=8)
    both = kv_cache_bytes(YARDSTICK, context=compressed_positions(32_768, block=4), kv_heads=2)
