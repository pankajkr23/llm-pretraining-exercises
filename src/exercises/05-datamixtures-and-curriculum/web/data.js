...
export const BUNDLE = Object.freeze({
 ...
 "lanes": [
  ...
  },
  {
   ...
   "share": 0.18,
   "baseline_share": 0.16,
   "because": "the differentiator, and the reason the project exists. Two points above the source default buys headroom over the 12% floor rather than sitting on it, at 1.33 epochs \u2014 inside the band where repetition is near-free",
   "schedule_only": false,
  ...
  },
  {
   ...
   "share": 0.12,
   "baseline_share": 0.12,
   "because": "unchanged, but on 146B of itemised supply rather than the 250B the source's supply check quotes. That moves it from 0.96 epochs to 1.64 \u2014 still fundable, with no margin left to give away",
   "schedule_only": false,
  ...
  },
  {
   ...
   "share": 0.02,
   "baseline_share": 0.02,
   "because": "held at the source's floor although supply cannot fund it: 40B against 627M is 3.9x more than infinite repetition could be worth. The share commits to *building* the data, not to holding it \u2014 priced in \u00a78",
   "schedule_only": false,
   ...
   "corrections": [
    {
     ...
     "because": "only the assistant's own tokens are supervised (\u00a76); at 0.3%-10.8% supervised per trajectory, derived from the token-per-sample counts of SWE-smith, OpenHands rollouts, SWE-Gym against the source's 'few hundred supervised tokens'. The generous end is applied, and the lane is impossible without the discount anyway",
