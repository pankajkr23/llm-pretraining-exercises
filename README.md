...
notebooks/hello.ipynb            # tracked sample; topic notebooks are built locally, not versioned
...
**Data conventions** — five concerns kept physically separate: requirement requirement documents are **never
tracked** (`REQUIREMENTS.md` is gitignored everywhere — a requirements document is the course's text and is input for
whoever builds the exercise, not the deliverable); topic notebooks live in `notebooks/`
...
- **The source material this work is built from is confidential and lives outside this repository.** Never name one of its files, quote it, or describe its contents in a tracked file — say what was decided and why instead. `tests/test_no_confidential_leaks.py` gates both on commit — the naming check also runs in CI, while the quoting check needs a machine that holds the material and skips elsewhere.
- **Some files exist only on your checkout, and git cannot restore them.** Topic notebooks, their builders and every `REQUIREMENTS.md` are gitignored. `uv run python tools/backup_local_only.py` snapshots them to a **git** store outside the repo (so every version is kept, not just the latest), and `uv run pytest tests/test_local_only_files_present.py` is the tripwire to run after any branch switch, pull, merge, rebase or stash. The authoritative list is `tools/backup_local_only.py::PATTERNS`.
...
and asserts no `REQUIREMENTS.md` is ever tracked. Introduce a shared `src/common/` package only once a
...
curriculum below. Full credit for the requirements, capstone projects, and dataset structures
