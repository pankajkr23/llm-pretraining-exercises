...
- **The source material this work is built from is confidential and lives outside this repository.** Never name one of its files, quote it, or describe its contents in a tracked file — say what was decided and why instead. `tests/test_no_confidential_leaks.py` enforces the first of those in CI and in a pre-commit hook; the second can only run on a machine that holds the material, so run the whole file before you push.
